...
admin.initializeApp({
  ...
  databaseURL: "https://loveisallaroundus-58e78.firebaseio.com"
	...
	var test1 = admin.database().ref("/");
	test1.limitToFirst(10).on("child_added", function(snapshot) {
		console.log('TWEETS IN TEST1' + snapshot.numChildren());
		console.log(snapshot.val());
		// test1.remove();
	});

	// var last = admin.database().ref('').limitToLast
// finishUp();
// function finishUp(){
	...
	postsRef.on('value', function(snapshot){
		...
		console.log(snapshot.child("-KY6wA95vOBRJDgNMbUM").val());
		// clearAllArrays();
 	});
...
function coordToValue(lat, lng ){
	...
	counter++;
		console.log('+1 COUNTER: ' + counter + ' TWEET LIBRARY LENGTH: ' + tweetLibrary.length);
		// debugger
